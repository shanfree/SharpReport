...
namespace Sirc.SharpReport.SQLServerDAL
{
    ...
    /// 数据访问类:VoyageFCL
    /// </summary>
    public partial class VoyageFCL : DBPersistenceBase<VoyageFCLInfo>, IVoyageFCL
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        #region sql
        private const string SQL_SELECT = @"SELECT * FROM V_VoyageFCL WHERE ID = @ID";
        private const string SQL_SELECT_VOYAGEID = @"SELECT * FROM V_VoyageFCL WHERE VoyageID = @VoyageID";
        #endregion
        ...
        public VoyageFCLInfo GetByVoyageID(string voyageID)
        {
            ...
            IList<VoyageFCLInfo> list = GetListFromReader(dr);
        ...
        public DataSet GetList(string dateID, string shipID, string reportType, int pageSize, int pageIndex)
        {
            ...
            {
                sql += string.Format(" and ManagerTypeID = {0}  ", reportType);
            }

            if (string.IsNullOrEmpty(shipID) == false)
            {
                ShipInfo sInfo = new Ship().GetByID(shipID);
                if (sInfo.LoadTypeEnum == ShipType.LCL)
                {
                    throw new Exception("该船为散货船舶，无集装箱报表信息。");
                }
        ...
        /// </summary>
        /// <returns>列表</returns>
        public IList<VoyageFCLInfo> GetUnReporList()
        {
            string sql = @"
                        select * from V_VoyageFCL
                        WHERE VoyageFCLID IS NULL ";
