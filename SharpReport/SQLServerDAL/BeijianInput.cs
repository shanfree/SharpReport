...
namespace Sirc.SharpReport.SQLServerDAL
{
    ...
    /// 数据访问类:BeijianInput
    /// </summary>
    public partial class BeijianInput : DBPersistenceBase<BeijianInputInfo>, IBeijianInput
    {
        public BeijianInput()
        { }

        #region sql
        private const string SQL_SELECT = @"SELECT * FROM BeijianInput WHERE ID = @ID";
        #endregion
        ...
        {
            string sql = @"
                        select dbo.V_Beijian.*, [User].Name as UserName from 
                        dbo.V_Beijian left join [User] on V_Beijian.InputUserID = [User].ID
                        WHERE 1 = 1 {0}";
        ...
        public DataSet GetCountStatictis(string year, string shipID)
        {
            string sql = @"
                                    ...
                                    isnull(
                                    (
                                        select sum(总数) from dbo.V_Beijian V
                                        where V.MonthNumOfYear = DT.MonthNumOfYear and V.Year = DT.Year 
		                                    {0}
                                    ),0) as 总数,                                  
                                    isnull(
                                    (
                                        select count(1) from dbo.V_Beijian V
                                        where V.MonthNumOfYear = DT.MonthNumOfYear and V.Year = DT.Year 
		                                    {0}
                                    ),0) as 全部,
                                    isnull(
        ...
        public DataSet GetSumStatictis(string year, string quarter)
        {
            string sql = @"
                                    ...
                                    isnull(
                                    (
                                        select SUM(主机) from dbo.V_Beijian V
                                        where 
                                            V.QuarterNumOfYear = @QuarterNumOfYear and V.Year = @Year and 
                                            V.ShipID = S.ID
                                            and V.UsageType = VI.UsageType
                                    ...
                                    isnull(
                                    (
                                        select SUM(副机) from dbo.V_Beijian V
                                        where 
                                            V.QuarterNumOfYear = @QuarterNumOfYear and V.Year = @Year and
                                            V.ShipID = S.ID
                                            and V.UsageType = VI.UsageType
                                    ...
                                    isnull(
                                    (
                                        select SUM(舾装) from dbo.V_Beijian V
                                        where 
                                            V.QuarterNumOfYear = @QuarterNumOfYear and V.Year = @Year and
                                            V.ShipID = S.ID
                                            and V.UsageType = VI.UsageType
                                    ...
                                    isnull(
                                    (
                                        select SUM(电器) from dbo.V_Beijian V
                                        where 
                                            V.QuarterNumOfYear = @QuarterNumOfYear and V.Year = @Year and
                                            V.ShipID = S.ID
                                            and V.UsageType = VI.UsageType
                                    ...
                                    isnull(
                                    (
                                        select SUM(辅机) from dbo.V_Beijian V
                                        where 
                                            V.QuarterNumOfYear = @QuarterNumOfYear and V.Year = @Year and
                                            V.ShipID = S.ID
                                            and V.UsageType = VI.UsageType
                                    ...
                                    isnull(
                                    (
                                        select SUM(分油机) from dbo.V_Beijian V
                                        where 
                                            V.QuarterNumOfYear = @QuarterNumOfYear and V.Year = @Year and
                                            V.ShipID = S.ID
                                            and V.UsageType = VI.UsageType
                                    ),0) as 分油机
                                    FROM dbo.Ship S,dbo.V_Beijian VI
