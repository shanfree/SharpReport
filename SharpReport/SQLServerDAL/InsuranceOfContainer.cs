...
{
    /// <summary>
    /// 数据访问类:InsuranceOfContainer
    /// </summary>
    public partial class InsuranceOfContainer : DBPersistenceBase<InsuranceOfContainerInfo>, IInsuranceOfContainer
    {
        public InsuranceOfContainer()
        { }

        #region sql
        private const string SQL_SELECT = @"SELECT * FROM InsuranceOfContainer WHERE ID = @ID";
        #endregion
        ...
        public DataSet GetList(string year)
        {
            ...
            string sql = @"
                        select dbo.V_InsuranceOfContainer.* from 
                        dbo.V_InsuranceOfContainer
                        WHERE Year=@Year ";
