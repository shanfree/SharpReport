<%@ Page Language="C#" MasterPageFile="~/MasterPage/Default.Master" AutoEventWireup="true"
    CodeBehind="JianyanSummary.aspx.cs" Inherits="SharpReportWeb.ChuanJ.JianyanSummary"
    Title="检验报表概况" %>
            ...
            <asp:GridView ID="gvJianyanList" runat="server" AutoGenerateColumns="False" Width="100%"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="报表数（全部）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="JianyanList.aspx?dimID=<%#Eval("ID") %>" target="_self">
                                <%#Eval("全部")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="报表数（预估录入）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="JianyanList.aspx?dimID=<%#Eval("ID")%>&ReportType=1" target="_self">
                                <%#Eval("预估录入")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="报表数（修正录入）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="JianyanList.aspx?dimID=<%#Eval("ID")%>&ReportType=2" target="_self">
                                <%#Eval("修正录入")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="报表数（财务确认）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="JianyanList.aspx?dimID=<%#Eval("ID")%>&ReportType=3" target="_self">
                                <%#Eval("财务确认")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
