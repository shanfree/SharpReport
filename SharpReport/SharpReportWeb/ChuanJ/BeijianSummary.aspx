<%@ Page Language="C#" MasterPageFile="~/MasterPage/Default.Master" AutoEventWireup="true"
    CodeBehind="BeijianSummary.aspx.cs" Inherits="SharpReportWeb.ChuanJ.BeijianSummary"
    Title="备件报表概况" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphTop" runat="server">
    ...
    <div class="window">
        <div class="title BGx">
            <span>备件报表报送情况</span></div>
        <div id="SlideBox">
            <asp:GridView ID="gvBeijianList" runat="server" AutoGenerateColumns="False" Width="100%"
                OnRowDataBound="gvBeijianList_RowDataBound" CssClass="t" PageSize="12">
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="报表数（全部）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="BeijianList.aspx?dimID=<%#Eval("ID") %>" target="_self">
                                <%#Eval("全部")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="报表数（预估录入）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="BeijianList.aspx?dimID=<%#Eval("ID")%>&ReportType=1" target="_self">
                                <%#Eval("预估录入")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="报表数（修正录入）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="BeijianList.aspx?dimID=<%#Eval("ID")%>&ReportType=2" target="_self">
                                <%#Eval("修正录入")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="报表数（财务确认）" ItemStyle-Width="20%">
                        <ItemTemplate>
                            <a href="BeijianList.aspx?dimID=<%#Eval("ID")%>&ReportType=3" target="_self">
                                <%#Eval("财务确认")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
