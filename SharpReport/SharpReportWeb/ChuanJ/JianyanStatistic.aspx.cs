...
{
    public partial class JianyanStatistic : WebBasePage
    {
        #region 权限验证
        /// <summary>
        /// 该页面登录后访问
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    TitleInitial("检验报表汇总统计", "栏目按照船舶汇总各个季度的检验报表的关键指标，用户可以点击下方的“查看报表构成”按钮查看指标的分解情况。");
                    BindYearList();
        ...
        protected void btnView_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url = "JianyanSummary.aspx?dimID=" + dimID;
