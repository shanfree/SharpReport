...
{
    public partial class XiuliList : WebBasePage
    {
        #region 权限验证
        /// <summary>
        /// 该页面登录后访问
        ...
        protected override void OnPreRender(EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    TitleInitial("修理报表汇总", "用户可以新增、修改或者删除修理报表信息，报表登记用户不能修改已经被财务部门确认过的报表。");
        ...
        /// <param name="e"></param>
        protected void gvXiuliList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                {
                    new XiuliInput().Delete(reportID);
                    BindList(0);
                    ShowMsg("删除成功");
                }
                else
                {
                    Response.Redirect("XiuliInputPage.aspx?ID=" + reportID, false);
                }
            }
            catch (ArgumentNullException aex)
            {
        ...
        protected void ExcelGenButton1_Click(object sender, EventArgs e)
        {
            try
            {
                int index = gvXiuliList.Columns.Count;
                gvXiuliList.Columns[index - 1].Visible = false;
                BindList(0, 5000);
            }
        ...
        protected void btnReturn_Click(object sender, EventArgs e)
        {
            try
            {
                string url = "XiuliSummary.aspx?dimID=" + this.DimID;
