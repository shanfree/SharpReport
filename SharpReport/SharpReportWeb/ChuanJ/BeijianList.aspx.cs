...
{
    public partial class BeijianList : WebBasePage
    {
        #region 权限验证
        /// <summary>
        /// 该页面登录后访问
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    TitleInitial("备件报表汇总", "用户可以新增、修改或者删除备件报表信息，报表登记用户不能修改已经被财务部门确认过的报表。");
                }
        ...
        #region 列表操作
        protected void gvBeijianList_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        /// <param name="e"></param>
        protected void gvBeijianList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                {
                    new BeijianInput().Delete(reportID);
                    BindList(0);
                    ShowMsg("删除成功");
                }
                else
                {
                    Response.Redirect("BeijianInputPage.aspx?ID=" + reportID, false);
                }
            }
            catch (ArgumentNullException aex)
            {
        ...
        protected void ExcelGenButton1_Click(object sender, EventArgs e)
        {
            try
            {
                int index = gvBeijianList.Columns.Count;
                gvBeijianList.Columns[index - 1].Visible = false;
                BindList(0, 5000);
            }
        ...
        protected void btnReturn_Click(object sender, EventArgs e)
        {
            try
            {
                string url = "BeijianSummary.aspx?dimID=" + this.DimID;
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url = "BeijianInputPage.aspx?dimID=" + dimID;
                Response.Redirect(url, false);
            }
            catch (ArgumentNullException aex)
            {
