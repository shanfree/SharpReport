...
{
    public partial class WuliaoList : WebBasePage
    {
        #region 权限验证
        /// <summary>
        /// 该页面登录后访问
        ...
        protected override void OnPreRender(EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    TitleInitial("物料报表汇总", "用户可以新增、修改或者删除物料报表信息，报表登记用户不能修改已经被财务部门确认过的报表。");
        ...
        #region 列表操作
        protected void gvWuliaoList_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        /// <param name="e"></param>
        protected void gvWuliaoList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                {
                    new WuliaoInput().Delete(reportID);
                    BindList(0);
                    ShowMsg("删除成功");
                }
                else
                {
                    Response.Redirect("WuliaoInputPage.aspx?ID=" + reportID, false);
                }
            }
            catch (ArgumentNullException aex)
            {
        ...
        protected void ExcelGenButton1_Click(object sender, EventArgs e)
        {
            try
            {
                int index = gvWuliaoList.Columns.Count;
                gvWuliaoList.Columns[index - 1].Visible = false;
                BindList(0, 5000);
            }
        ...
        protected void btnReturn_Click(object sender, EventArgs e)
        {
            try
            {
                string url = "WuliaoSummary.aspx?dimID=" + this.DimID;
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url = "WuliaoInputPage.aspx?dimID=" + dimID;
                Response.Redirect(url, false);
            }
            catch (ArgumentNullException aex)
            {
